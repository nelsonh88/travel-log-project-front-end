...
var welcomeText = function welcomeText(data) {
  var newLede = 'Welcome ' + data.user.first_name + '! Below you will find a listing of your previously logged past trips and a button to create a new trip.';
  $('.lede').text(newLede);
};

var byeText = function byeText() {
  var newLede = 'Thank you, you have been successfully logged out. Please continue traveling with us!';
  $('.lede').text(newLede);
};
...
var getTrips = function getTrips() {
  return $.ajax({
    url: config.apiOrigin + '/trips',
    method: 'GET',
    headers: {
...
var updateTrip = function updateTrip(data) {
  return $.ajax({
...
var createTripSuccess = function createTripSuccess(data) {
...
var getTripsSuccess = function getTripsSuccess(data) {
  ...
  var tabletgt = document.getElementById('tabletarget');
  ...
  data.trips.forEach(function (item) {
    ...
    var cell1text = document.createTextNode(item.id);
    cell1.appendChild(cell1text);
    row.appendChild(cell1);
    var cell2 = document.createElement('td');
    var cell2text = document.createTextNode(item.location_name);
    cell2.appendChild(cell2text);
    row.appendChild(cell2);
    var cell3 = document.createElement('td');
    var cell3text = document.createTextNode(item.date);
    cell3.appendChild(cell3text);
    row.appendChild(cell3);

    var cell4 = document.createElement('td');
    var cell4text = document.createTextNode(item.travel_mates);
    cell4.appendChild(cell4text);
    row.appendChild(cell4);

    var cell5 = document.createElement('td');
    var cell5text = document.createTextNode(item.description);
...
var getTripsFailure = function getTripsFailure() {
  // console.log(error)
  $('#message').text('Error on getting trips!');
  $('#message').removeClass('alert-success').addClass('alert-danger').show();
};

var getTripSuccess = function getTripSuccess(data) {
...
var updateTripSuccess = function updateTripSuccess(data) {
...
var deleteTripSuccess = function deleteTripSuccess(data) {
...
module.exports = {
  ...
  getTripsFailure: getTripsFailure,
  getTripSuccess: getTripSuccess,
...
var config = __webpack_require__(1);
var authEvents = __webpack_require__(13);
var tripEvents = __webpack_require__(15);
...
var ui = __webpack_require__(5);
var tripsApi = __webpack_require__(6);
var tripsUi = __webpack_require__(7);
...
var onSignIn = function onSignIn() {
  ...
  // event.target is the same as this in the parameter below
  var data = getFormFields(this);
  api.signIn(data).then(ui.signInSuccess).then(tripsApi.getTrips).then(tripsUi.getTripsSuccess).catch(ui.signInFailure);
};
...
var onCreateTrip = function onCreateTrip(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.createTrip(data).then(ui.createTripSuccess).then(api.getTrips).then(ui.getTripsSuccess).catch(ui.createTripFailure);
};

var onGetTrips = function onGetTrips(event) {
  event.preventDefault();
  api.getTrips().then(ui.getTripsSuccess).catch(ui.getTripsFailure);
};

var onGetTrip = function onGetTrip(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  var trip = data.trip;
  // console.log(event)
  api.getTrip(trip.id).then(ui.getTripSuccess).catch(ui.getTripFailure);
};

var onUpdateTrip = function onUpdateTrip(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.updateTrip(data).then(ui.updateTripSuccess).then(api.getTrips).then(ui.getTripsSuccess).catch(ui.updateTripFailure);
};
...
var onDeleteTrip = function onDeleteTrip(event) {
  ...
  var data = getFormFields(event.target);
  var trip = data.trip;
  api.deleteTrip(trip.id).then(ui.deleteTripSuccess).then(api.getTrips).then(ui.getTripsSuccess).catch(ui.deleteTripFailure);
};
